...
- `usage_request_tool_attribution` joins tool attribution to request context by `model + bucket + session_id + prompt_index`, adding `request_pattern`, `request_subpattern`, and `task_label` for command-cost and session-cost breakdowns.
