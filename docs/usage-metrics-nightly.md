...
- `usage_request_cache_diagnosis` emits one row per prompt/request with exact per-request cache-read, cache-creation, output, reasoning, total token, cache-hit, and derived cost fields. It also labels the request pattern and a primary likely cache driver.
...
- Request diagnosis rows are marked with `diagnosis_version = request_cache_sources_v2`; this version is included in diagnosis `$insert_id` keys so future diagnosis-schema changes do not collide with earlier imports.
