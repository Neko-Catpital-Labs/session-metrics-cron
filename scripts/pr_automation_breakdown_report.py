#!/usr/bin/env python3
"""Break down automated PR-repair cost and retry thrash.

Reads the prompt-row CSV emitted by scripts/planning_vs_execution_report.py
and produces JSON, Markdown, and HTML reports of automated PR-repair cost,
grouped by category and by the (repo, PR) target being repaired.

A "retry-thrash" target is a (repo, PR) that was repaired across more than
one session. Each thrash target's JSON entry carries an `attempts_detail`
list identifying the individual sessions behind its `attempts` count via
`session_id` (a JSONL filename stem, i.e. Path(file).stem) and
`prompt_index` -- exactly the identifiers scripts/splitter_metric_tree_app.py's
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Break down automated PR-repair cost and retry thrash.")
...
def build_window(rows: list[dict[str, str]], duration_cap_seconds: float = 3600.0) -> dict[str, Any]:
    ...
    # (repo, pr_number) -> session_id -> per-session record (cost_usd, session_date, host, prompt_index, best_cost)
