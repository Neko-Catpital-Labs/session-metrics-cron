...
Each thrash target's JSON entry carries an attempts_detail list identifying the
individual sessions behind it (session_id, prompt_index, cost_usd, session_date,
host). session_id is the session's JSONL filename without its extension or
directory (Path(file).stem) and prompt_index is the original CSV value -- the
same (session_id, prompt_index) pair accepted by the existing
/api/cost-explorer-window endpoint in scripts/splitter_metric_tree_app.py. A
later, separate workflow uses those identifiers to make the HTML report
interactive.
"""
...
# Fields searched (in order, first match wins) for a GitHub PR reference.
PR_REF_TEXT_FIELDS = (
    "target",
    "session_cwd",
    "prompt_preview",
...
DURATION_FIELDS = ("active_seconds", "duration_seconds", "elapsed_seconds")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Number of retry-thrash targets kept in thrash.top[] (Markdown output uses the same cap).",
    )
    parser.add_argument(
        "--duration-cap-seconds",
        type=float,
        default=3600.0,
        help="Per-row cap applied when accumulating active_seconds; raw_seconds is always uncapped.",
    )
...
def _empty_category_fact_row(name: str) -> dict[str, Any]:
    return {"category": name, "cost_usd": 0.0, "active_seconds": 0.0, "prompts": 0, "raw_seconds": 0.0}


def _finalize_category_fact_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
...
def build_window(rows: list[dict[str, str]], duration_cap_seconds: float = 3600.0) -> dict[str, Any]:
    ...
    # (repo, pr_number) -> session_id -> per-session record.
    ...
    for row in rows:
        ...
        category = categories.setdefault(category_name, _empty_category_fact_row(category_name))
    ...
    category_rows = sorted(
        (_finalize_category_fact_row(row) for row in categories.values()),
    ...
    thrash_targets.sort(
        key=lambda item: (-item["cost_usd"], -item["attempts"], item["repo"], item["pr_number"])
    )
...
def build_report(rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    # thrash_targets stays the full, uncapped list; only thrash.top[] (and the
    # Markdown output, which reads thrash.top) is sliced to --top-thrash.
...
def render_markdown(data: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    top_thrash = to_int(getattr(args, "top_thrash", 25))
    lines = [
