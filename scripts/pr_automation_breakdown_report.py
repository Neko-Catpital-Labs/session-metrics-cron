...
Consumes the planning/cost CSV rows emitted by
scripts/planning_vs_execution_report.py and produces JSON, Markdown, and
HTML summaries centered on PR repair automation: overall cost by category,
and which PRs are getting repeatedly re-repaired ("retry thrash").

The JSON output is the schema downstream tooling (Cost Explorer, the
interactive HTML report) reads from. Each retry-thrash target's JSON entry
carries an `attempts_detail` list identifying the individual sessions
behind its `attempts` count -- `session_id` (a JSONL filename stem) and
`prompt_index` are exactly what scripts/splitter_metric_tree_app.py's
        ...
        (
            {
                ...
                "active_seconds": round(row["active_seconds"], 4),
                "prompts": row["prompts"],
                "raw_seconds": round(row["raw_seconds"], 4),
            }
