...
(reports/planning-vs-execution-prompts.csv) and writes Markdown, JSON, and
HTML reports. Prompt rows that reference a GitHub PR are grouped into
"retry-thrash" targets -- one per (repo, pr_number) pair -- so PRs that
needed many automated repair sessions (for example PR #6550, repaired 48
times) are easy to spot.

Each thrash target's JSON entry carries an attempts_detail list identifying
the individual sessions behind it: session_id (the session's JSONL filename
without its extension or directory, i.e. Path(file).stem), prompt_index (the
original CSV value), cost_usd, session_date, and host. That
...
def build_window(rows: list[dict[str, str]], duration_cap_seconds: float = DEFAULT_DURATION_CAP_SECONDS) -> dict[str, Any]:
    ...
    # (repo, pr_number) -> session_id -> per-session record. prompt_index is
    # kept as the original CSV string from whichever row (for that session,
    # against this target) has the highest cost_usd seen so far.
...
def build_report(rows: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    # thrash_targets[] stays the full, uncapped list of every distinct
    # (repo, pr_number) target; only thrash.top[] (which render_markdown()
    # reads) is sliced to --top-thrash.
