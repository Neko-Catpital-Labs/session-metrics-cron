...
Consumes the local planning/cost CSV rows emitted by
scripts/planning_vs_execution_report.py and produces JSON, Markdown, and HTML
summaries centered on PR repair automation: overall cost by category, and
which PRs are getting repeatedly re-repaired ("retry thrash").

The JSON output is the schema downstream tooling (Cost Explorer, the
interactive HTML report) should read from; Markdown/HTML are static
read-only summaries capped to the top `--top-thrash` targets.
"""
...
PR_REFERENCE_PATTERNS = (
    ...
    re.compile(r"\bpull\s+request\s+#?(?P<pr>\d+)\b(?:\s+(?:in|for|on)\s+(?P<repo>[\w.-]+/[\w.-]+))?", re.IGNORECASE),
)

PR_REFERENCE_TEXT_FIELDS = (
    "target",
    "prompt_preview",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--input", default=str(DEFAULT_INPUT), help="Input CSV of prompt rows.")
    parser.add_argument("--markdown-out", default=str(DEFAULT_MARKDOWN_OUT))
    parser.add_argument("--json-out", default=str(DEFAULT_JSON_OUT))
    parser.add_argument("--html-out", default=str(DEFAULT_HTML_OUT))
    parser.add_argument("--top-thrash", type=int, default=25, help="Number of thrash targets shown in Markdown/HTML.")
    parser.add_argument(
        "--duration-cap-seconds",
        type=float,
        default=3600.0,
        help="Maximum per-row duration counted toward active_seconds. raw_seconds stays uncapped.",
    )
...
def row_cost_usd(row: dict[str, Any]) -> float:
    for key in ("cost_usd", "derived_total_cost_usd", "allocated_total_cost_usd", "estimated_cost_usd"):
...
def extract_repo_and_pr(row: dict[str, Any]) -> tuple[str, int] | None:
    ...
    text = " ".join(str(row.get(field) or "") for field in PR_REFERENCE_TEXT_FIELDS)
    for pattern in PR_REFERENCE_PATTERNS:
...
def category_for_row(row: dict[str, Any]) -> str:
    for key in ("category", "fixing_cause", "task_type_label", "task_type", "request_pattern_path", "request_pattern"):
...
def is_thrash_row(row: dict[str, Any]) -> bool:
    explicit = str(row.get("efficiency_label") or "").strip().lower()
    if explicit == "thrash":
        return True
...
def build_window(rows: list[dict[str, str]], duration_cap_seconds: float = 3600.0) -> dict[str, Any]:
    ...
    thrash_targets.sort(key=lambda item: (-to_float(item["cost_usd"]), -to_int(item["attempts"]), item["repo"], item["pr_number"]))
