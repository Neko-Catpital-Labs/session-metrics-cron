...
class SplitterMetricTreeAppTests(unittest.TestCase):
    ...
    def test_fixing_and_explorer_dashboards_include_new_sections(self) -> None:
        ...
        self.assertIn('Filter field', explorer_html)
        self.assertIn('Filter value', explorer_html)
        self.assertIn('request pattern', explorer_html)
        self.assertIn('Filter field picks the label family.', explorer_html)
        ...
        self.assertIn('.label-evidence { display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:.65rem; margin:.8rem 0; }', explorer_html)
        ...
        self.assertIn('function activeTaskTypeFilter(){', explorer_html)
        self.assertIn('function chunkMatchesTaskType(payload, chunk, commandRows = payload.commands || []){', explorer_html)
        self.assertIn('.chunk-card.is-task-match { border-color:#c7d2fe; box-shadow: inset 3px 0 0 #6366f1; }', explorer_html)
        self.assertIn('.chunk-meta .task-match-badge { color:#312e81; background:#e0e7ff; border-radius:999px; padding:.08rem .42rem; font-size:.72rem; }', explorer_html)
        self.assertIn('Step summary', explorer_html)
        self.assertIn('card.classList.toggle(\'is-task-match\', chunkMatchesTaskType(payload, chunk, commandRows));', explorer_html)
        self.assertIn('<span class="task-match-badge" ${isTaskMatch ? \'\' : \'hidden\'}>task match</span>', explorer_html)
        self.assertIn('const taskMatch = taskType ? timelineEntries.find(({ chunk }) => chunkMatchesTaskType(payload, chunk, visibleCommands)) : null;', explorer_html)
        self.assertIn('const isTaskMatch = chunkMatchesTaskType(payload, chunk, visibleCommands);', explorer_html)
