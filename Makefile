...
.PHONY: audit report export-dry-run warehouse-demo-validate nightly-dry-run test benchmark-dry-run lint
...
warehouse-demo-validate:
	$(PYTHON) scripts/warehouse_cost_demo.py validate-local --expect-full-row-count
...
test:
	...
	$(PYTHON) scripts/test-pr-automation-breakdown-report.py
	$(PYTHON) scripts/test-invoker-skill-run-timing-report.py
	$(PYTHON) scripts/test-invoker-skill-run-backfill.py
