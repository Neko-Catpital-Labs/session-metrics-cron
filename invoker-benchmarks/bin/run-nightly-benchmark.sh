...
sync_runtime_to_worker() {
  ...
  if [[ -d "$BENCHMARK_ROOT/scripts" ]]; then
    ssh -p "$port" "$target" "mkdir -p '$BENCHMARK_ROOT/scripts'"
    rsync -az -e "ssh -p $port" "$BENCHMARK_ROOT/scripts/" "$target:$BENCHMARK_ROOT/scripts/"
  else
    local costing_source=""
    for candidate in "$SCRIPT_DIR/../../scripts/usage_costing.py"; do
      if [[ -f "$candidate" ]]; then
